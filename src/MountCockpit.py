...
class MountCockpit():
	...
	def getMountPoint(self, plugin, path):
		...
		if bookmark in self.mounts_table:
			...
			logger.error("no bookmark found for: %s", path)
	...
	def getBookmarksSpaceInfo(self, plugin):
		...
		short_bookmarks = []
		for __bookmark in self.bookmarks[plugin]:
			if self.getMountPoint(plugin, __bookmark) == __bookmark:
				short_bookmark = __bookmark[len("/media/"):]
			else:
				short_bookmark = os.path.dirname(__bookmark)[len("/media/"):]
			if short_bookmark and short_bookmark not in short_bookmarks:
